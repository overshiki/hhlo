...
runBuilder name args' builderAction =
    let Builder m = builderAction
        initState = BuildState (length args') [] (length args')
        (Tensor finalVid, finalState) = runState m initState
        -- The result type should match the final tensor's type.
        -- Since the Tensor is phantom-typed, we can reconstruct the type.
...
moduleFromBuilder name args' action = Module [runBuilder name args' action]

-- | Emit a generic operation into the builder.
emitOp :: Text -> [ValueId] -> [Attribute] -> TensorType -> Builder ValueId
emitOp name operands attrs resultType = do
    n <- gets bsNextId
    let vid = ValueId n
    modify $ \s -> s
        { bsNextId = n + 1
        , bsOps = Operation name operands attrs vid resultType : bsOps s
        }
    return vid
